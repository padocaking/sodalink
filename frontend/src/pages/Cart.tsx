...
function CartRow({ item, onQtyChange }: { item: CartItem & { isRemoving?: boolean }; onQtyChange: (qty: number) => Promise<void> }) {
  ...
  const changeQty = (next: number) => {
    ...
    setIsUpdating(true);
    onQtyChange(next).finally(() => setTimeout(() => setIsUpdating(false), 100));
  };
      ...
      <div className="mt-3 flex items-end justify-between">
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={isUpdating}
            onClick={(e) => { e.preventDefault(); changeQty(quantity - 1); }}
          ...
          <span className="w-14 text-center text-sm font-semibold text-gray-800 border border-gray-200 rounded-lg py-0.5">
            {quantity}
          </span>
          <button
            aria-label="Aumentar"
        ...
        <div className="text-right shrink-0">
          ...
          {product.comparePrice ? (
            <p className="text-[0.6rem] text-gray-400 line-through">{formatPrice(product.comparePrice)}</p>
          ) : (
            <p className="text-[0.6rem] text-gray-500">({formatPrice(product.price)} / un)</p>
          )}
        </div>
      </div>
    </div>
